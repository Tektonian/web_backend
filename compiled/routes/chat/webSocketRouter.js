...
const ResMessageRactory = (message, sender, lastReadSequences) => {
    let ret;
    let cnt = 0;
...
const ResMessagesFactory = (messages, sender, lastReadSequences) => {
...
async function sendMessageHandler(globalArgs, recv) {
    ...
    await chatContentController.sendMessage(chatRoom._id, chatUser, req.message.content);
}
...
async function socketDisconnectHandler(globalArgs, reason) {
    const { socket, userSentEvent, updateChatRoomEvent, chatUserController } = globalArgs;
    const chatUser = socket.data.chatUser;
    console.log("User disconnected: ", socket.data.chatUser, " reason ", reason);
    await chatUserController.delUserById(chatUser._id);
}
async function userSentEventHandler(globalArgs, { jobId, returnvalue, // JSON.stringfied IChatContent
 }) {
    ...
    const responses = await io
        ...
        .timeout(500)
        .emitWithAck("someoneSent", JSON.stringify(ResMessageRactory(JSON.parse(returnvalue), chatUser, [0])));
    const respondUserIds = responses.map((res) => res.id);
    chatUnreadController.whetherSendAlarm(chatRoom, JSON.parse(returnvalue), participant_ids, respondUserIds);
...
async function userTryJoinHandler(globalArgs, req) {
    const { socket, chatRoomController, chatUser, chatContentController, chatUnreadController, } = globalArgs;
    // Is he ok to join?
    ...
    if (currChatRoomSeq - deviceLastSeq > 0) {
        // get unread messages
        const unreadMessages = await chatContentController.getChatRoomMessagesBySeq(chatRoom, deviceLastSeq);
        messages.push(...unreadMessages);
        // update unread schema
        await chatUnreadController.updateUserUnread(chatUser, chatRoom, currChatRoomSeq);
        // console.log("Unread, ", unreadMessages);
    }
    // get last read sequences
    const lastReadSeqences = await chatUnreadController.getUnreadSequences(chatRoom._id);
    const resMessages = ResMessagesFactory(messages, chatUser, lastReadSeqences);
...
const eventRegistHelper = async (eventFlow) => {
    ...
    for (let chain of chains) {
        ...
        const eventName = typeof chain.eventName === "function"
            ? chain.eventName()
            : chain.eventName;
        const action = chain.action;
        const nextChains = chain.chains;
        const nextEventFlow = {
        ...
        // Regist events
            ...
            // Check recursive
            if (nextChains === undefined) {
                await target.on(eventName, chain.handler.bind(target, globalArgs));
            }
            else {
...
export default function initChat(httpServer) {
    ...
    io.on("connection", async (socket) => {
        ...
        const chatUser = await chatUserController.createUser({
            ...
            chains: [
                {
                    ...
                    chains: [
                        {
                            description: "After user joined a chatroom, the `socket.data.chatRoom` should be set so we will evaluate eventName later",
                            eventTarget: "userSentEvent",
                            eventName: () => `${socket.data.chatRoom._id.toString()}:${socket.data.chatUser._id.toString()}`,
                            handler: userSentEventHandler,
                        },
                    ],
                },
                {
                    description: "When user leave a chatroom, make user leave a socket room and remove eventlistenr",
                    eventTarget: "socket",
                    eventName: "userTryUnjoin",
                    handler: userTryUnJoinHandler,
                    chains: [
                        {
                            description: "remove eventlistener of userSentEvent",
                            eventTarget: "userSentEvent",
                            action: "off",
                            eventName: () => `${socket.data.chatUser._id.toString()}:${socket.data.chatRoom._id.toString()}`,
                        },
                    ],
                },
                {
