...
CREATE  OR REPLACE VIEW `FullStudentProfile` AS
    ...
    FROM
        ...
        LEFT JOIN (SELECT 
            JSON_ARRAYAGG(JSON_OBJECT('exam_result', `examhistorywithlanguageexam`.`exam_result`, 'exam_name_glb', `examhistorywithlanguageexam`.`exam_name_glb`,'exam_results', `examhistorywithlanguageexam`.`exam_results`, 'lang_country_code', `examhistorywithlanguageexam`.`lang_country_code`)) AS `language`,
