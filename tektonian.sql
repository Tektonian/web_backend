...
CREATE TABLE IF NOT EXISTS `tektonian`.`Corporation` (
  ...
  `corp_num` INT UNSIGNED NOT NULL,
  `biz_num` INT UNSIGNED NULL,
...
CREATE TABLE IF NOT EXISTS `tektonian`.`CorporationReview` (
  ...
  `work_atmosphere` VARCHAR(255) NULL,
  `sense_of_achive` TINYINT NULL,
  `work_intensity` TINYINT NULL,
  `pay_satisfaction` TINYINT NULL,
  `CorporationReviewcol` VARCHAR(45) NULL,
  `created_at` DATETIME NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` DATETIME NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`));
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `tektonian`.`RequestOfUser` (`user_id` INT, `username` INT, `image` INT, `consumer_type` INT, `consumer_email` INT, `phone_number` INT, `request_id` INT, `consumer_id` INT, `title` INT, `subtitle` INT, `head_count` INT, `reward_price` INT, `currency` INT, `content` INT, `are_needed` INT, `are_required` INT, `start_date` INT, `end_date` INT, `address` INT, `address_coordinate` INT, `provide_food` INT, `provide_trans_exp` INT, `prep_material` INT, `created_at` INT, `status` INT, `start_time` INT, `end_time` INT);

-- -----------------------------------------------------
-- Placeholder table for view `tektonian`.`UserOfCorporation`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `tektonian`.`UserOfCorporation` (`user_id` INT, `username` INT, `email` INT, `image` INT, `roles` INT, `corp_id` INT, `corp_name` INT, `nationality` INT, `corp_domain` INT, `ceo_name` INT, `corp_address` INT, `phone_number` INT, `corp_num` INT, `biz_num` INT, `biz_started_at` INT, `status` INT, `biz_type` INT, `logo_image` INT, `site_url` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `tektonian`.`RequestOfCorporation` (`request_id` INT, `consumer_id` INT, `title` INT, `subtitle` INT, `head_count` INT, `reward_price` INT, `currency` INT, `content` INT, `are_needed` INT, `are_required` INT, `start_date` INT, `end_date` INT, `address` INT, `address_coordinate` INT, `provide_food` INT, `provide_trans_exp` INT, `prep_material` INT, `created_at` INT, `status` INT, `start_time` INT, `end_time` INT, `corp_id` INT, `corp_name` INT, `corp_domain` INT, `nationality` INT, `corp_address` INT, `phone_number` INT, `logo_image` INT, `site_url` INT);

-- -----------------------------------------------------
-- Placeholder table for view `tektonian`.`ExamHistoryWithLanguageExam`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `tektonian`.`ExamHistoryWithLanguageExam` (`student_id` INT, `exam_result` INT, `exam_id` INT, `exam_name_glb` INT, `exam_result_type` INT, `exam_results` INT, `exam_level` INT, `lang_country_code` INT);

-- -----------------------------------------------------
-- Placeholder table for view `tektonian`.`FullStudentProfile`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `tektonian`.`FullStudentProfile` (`student_id` INT, `user_id` INT, `name_glb` INT, `nationality` INT, `age` INT, `created_at` INT, `email_verified` INT, `phone_number` INT, `emergency_contact` INT, `gender` INT, `image` INT, `has_car` INT, `keyword_list` INT, `academic` INT, `language` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `tektonian`.`StudentWithCurrentSchool` (`user_id` INT, `name_glb` INT, `nationality` INT, `age` INT, `student_phone_number` INT, `emergency_contact` INT, `gender` INT, `image` INT, `has_car` INT, `keyword_list` INT);
...
CREATE  OR REPLACE VIEW `ExamHistoryWithLanguageExam` AS
    SELECT 
        ...
        `l`.`exam_result_type` AS `exam_result_type`,
        `l`.`exam_results` AS `exam_results`,
        `l`.`exam_level` AS `exam_level`,
...
CREATE  OR REPLACE VIEW `FullStudentProfile` AS
    ...
    FROM
        ...
        LEFT JOIN (SELECT 
            JSON_ARRAYAGG(JSON_OBJECT('exam_result', `examhistorywithlanguageexam`.`exam_result`, 'exam_name_glb', `examhistorywithlanguageexam`.`exam_name_glb`, 'exam_result_type', `examhistorywithlanguageexam`.`exam_result_type`, 'exam_results', `examhistorywithlanguageexam`.`exam_results`, 'exam_level', `examhistorywithlanguageexam`.`exam_level`, 'lang_country_code', `examhistorywithlanguageexam`.`lang_country_code`)) AS `language`,
