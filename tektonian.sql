...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `tektonian`.`RequestOfCorporation` (`request_id` INT, `consumer_id` INT, `title` INT, `subtitle` INT, `head_count` INT, `reward_price` INT, `currency` INT, `content` INT, `are_needed` INT, `are_required` INT, `start_date` INT, `end_date` INT, `address` INT, `address_coordinate` INT, `provide_food` INT, `provide_trans_exp` INT, `prep_material` INT, `created_at` INT, `request_status` INT, `start_time` INT, `end_time` INT, `corp_id` INT, `corp_name` INT, `corp_domain` INT, `nationality` INT, `corp_address` INT, `phone_number` INT, `logo_image` INT, `site_url` INT);
